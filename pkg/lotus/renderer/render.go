...
// RenderNode renders the node tree to terminal using ANSI codes
func RenderNode(root *core.Node) string {
...
func renderNode(node *core.Node, buf *strings.Builder) {
	if node == nil {
		return
	}
...
func renderBorder(node *core.Node, buf *strings.Builder) {
...
func renderText(node *core.Node, buf *strings.Builder) {
	// Get content from this node or its first child (parser creates nested text nodes)
	text := node.Content
	if text == "" && len(node.Children) > 0 && node.Children[0].Type == "text" {
...
func renderInput(node *core.Node, buf *strings.Builder) {
	prompt := node.Attributes["prompt"]
	if prompt == "" {
		prompt = "> "
	}

	// Get placeholder from node content or first child
	placeholder := node.Content
	if placeholder == "" && len(node.Children) > 0 && node.Children[0].Type == "text" {
		placeholder = node.Children[0].Content
	}

	color := getANSIColor(node.Styles.Color)

	// Position cursor for input (node.X and node.Y are already final positions from layout)
	moveCursor(node.X, node.Y, buf)
	buf.WriteString(color)
	...
	// Position cursor after prompt for typing
	moveCursor(node.X+len(prompt), node.Y, buf)
}
