...
func (k KeyEvent) IsPrintable() bool {
	// Check if Char field is set (for test events)
	if k.Char != "" {
		return true
	}
...
// Cancel cancels the input reader
func (r *InputReader) Cancel() bool {
	return r.reader.Cancel()
}
...
func (r *InputReader) ReadKey() (*KeyEvent, error) {
	...
	if r.inEscapeSeq {
		...
		if seq == SeqCtrlLeft || seq == SeqAltLeft {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqCtrlLeft}, nil
		}
		if seq == SeqCtrlRight || seq == SeqAltRight {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqCtrlRight}, nil
		}
		if seq == SeqCmdLeft {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqCmdLeft}, nil
		}
		if seq == SeqCmdRight {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
		...
		if seq == SeqLeft {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqLeft}, nil
		}
		if seq == SeqRight {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqRight}, nil
		}
		if seq == SeqUp {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqUp}, nil
		}
		if seq == SeqDown {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqDown}, nil
		}
		if seq == SeqHome || seq == SeqHome2 {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqHome}, nil
		}
		if seq == SeqEnd || seq == SeqEnd2 {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqEnd}, nil
		}
		if seq == SeqDelete {
			r.inEscapeSeq = false
			r.escapeBuffer = r.escapeBuffer[:0]
			return &KeyEvent{Key: KeyEscape, Code: SeqDelete}, nil
		}
