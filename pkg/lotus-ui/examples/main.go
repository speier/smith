...
import (
	...
	ui "github.com/speier/smith/pkg/lotus-ui"
)
...
func NewKitchenSink() *KitchenSink {
	...
	app.tabs = ui.NewTabs().
		...
		WithActive(0).
		WithTabBarStyle(ui.TabBarStyleLine)
...
func NewFormsDemo() *FormsDemo {
	demo := &FormsDemo{
		message: ui.NewTextBox().WithAutoScroll(true),
	}

	demo.name = ui.NewTextInput().
		WithID("name-input").
...
func NewSelectDemo() *SelectDemo {
	...
	demo.country = ui.NewSelect().
		WithID("country-select").
		WithOptions([]ui.SelectOption{
...
type ProgressDemo struct {
	progress1 *ui.ProgressBar
	progress2 *ui.ProgressBar
	progress3 *ui.ProgressBar
}

func NewProgressDemo() *ProgressDemo {
	progress1 := ui.NewProgressBar(40)
	progress1.SetValue(0.25)

	progress2 := ui.NewProgressBar(40)
	progress2.SetValue(0.65)

	progress3 := ui.NewProgressBar(40)
